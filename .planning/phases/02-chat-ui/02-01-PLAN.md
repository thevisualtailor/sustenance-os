...
<objective>
Build the core chat view — flex column layout, auto-expanding input field, message thread container, empty state branding, and in-memory message store with stub AI responses.
...
4. Empty state: Create a `.chat-empty` div centered in the message thread area containing:
   ...
   - Both elements get a CSS transition (opacity 0.3s ease) and are hidden (opacity: 0, pointer-events: none) once messages exist
5. Append the `.chat-view` to the provided container

**src/chat/chat-view.css** — all chat layout styles:
- `.chat-view`: `height: calc(100dvh - var(--header-height, 60px)); display: flex; flex-direction: column;` — DO NOT use `position: fixed` (broken on iOS Safari per research)
...
- `.chat-empty`: centered in thread area using flex, vertically and horizontally
...
**src/main.js** — replace the placeholder with chat view:
- Remove the placeholder div creation code
- Import `createChatView` from `./chat/chat-view.js`
- Call `createChatView(main)` where `main` comes from `createAppShell(app)`
...
1. Chat view fills the screen below the header
2. Empty state shows "SustenanceOS" and "Persistent Imperfection" centered
3. Textarea starts as single line, grows when typing multiple lines, caps at 4 lines
4. Enter key adds a newline in the textarea (does NOT send)
5. Send button is visible and tappable (44px+ tap target)
6. `npm run build` succeeds without errors
  </verify>
  <done>
Chat layout renders correctly with flex column (thread + input bar), auto-expanding textarea works with Enter=newline, empty state branding is visible and centered, build passes.
  </done>
</task>
...
2. On send button click (or form submit):
   ...
   h. Hide the empty state (add `.chat-empty--hidden` class)
...
1. Type a message and tap Send — user message appears right-aligned
2. "Thinking..." animation appears with pulsing dots
3. After ~1.5 seconds, stub AI response appears full-width
4. Empty state ("SustenanceOS" / "Persistent Imperfection") disappears after first message
5. Can send multiple messages — all appear in thread
6. Textarea clears and resets height after sending
7. `npm run build` succeeds
  </verify>
  <done>
User can type a message, tap send, see it appear right-aligned, watch the thinking animation, and receive a styled stub AI response full-width. Empty state hides after first message. Messages accumulate in the in-memory store.
  </done>
</task>
...
3. Empty state visible on fresh load, hidden after first message
...
- Empty state branding ("SustenanceOS" / "Persistent Imperfection") visible on load, fades when messages exist
